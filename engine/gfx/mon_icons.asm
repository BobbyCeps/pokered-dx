...
.animateSprite
	...
	ld a, [wCurrentMenuItem]
	call AddNTimes
;	ld c, ICONOFFSET / REMOVED for unique party icon
;	ld a, [hl]
;	cp ICON_BALL << 2
;	jr z, .editCoords
;	cp ICON_HELIX << 2
...
;.editTileIDS
	ld c, $2 ;added for unique party icon
	ld b, $4
	ld de, $4
.loop
	ld a, [hl]
...
PUSHS ; New code for unique party menu sprite icon
...
LoadSinglePartyMonSprite:
	...
	farcall LoadSinglePartySpritePalette ;fix party mon sprite
	call EnableLCD
...
; and backup wShadowOAM into wMonPartySpritesSavedOAM
	...
	PlaceSinglePartyMonSprite: ;fix party mon sprite
	push hl
	push de
	push bc
...
IF ALT_PARTY_MENU_COLOR ;fix party mon sprite
	ld a, [hPartyMonIndex]
